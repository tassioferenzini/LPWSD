 ...
 * @author tassio
 */
public class AssuntoDAOHibernate implements Serializable {

    Logger logger = Logger.getLogger("DAO");
    public static AssuntoDAOHibernate assuntoDAO;

    public static AssuntoDAOHibernate getInstance() {
        if (assuntoDAO == null) {
            assuntoDAO = new AssuntoDAOHibernate();
        }
        return assuntoDAO;
    }
    ...
    public TbAssunto getTbAssunto(int id) {
        ...
        try {
            session = (Session) Conexao.getSession();
            TbAssunto tbAssunto = (TbAssunto) 
                    session.get(TbAssunto.class, new Integer(id));
            session.close();
            return tbAssunto;
        } catch (Exception ex) {
    ...
    public TbAssunto deleteTbAssunto(int id) {
        ...
        try {
            session = (Session) Conexao.getSession();
            TbAssunto tbAssunto = 
                    (TbAssunto) session.get(
                            TbAssunto.class, new Integer(id));
            tx = session.beginTransaction();
